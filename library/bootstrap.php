...
$path = ROOT . DS . 'configs' . DS . 'config.php';
...
spl_autoload_register(function ($className) {
    ...
    if (!$valid) {
        $valid = file_exists($classFile = $rootPath . 'app' . DS . 'controllers' . DS . $className . '.php');
    }
    // if we cannot find any, then find application/models directory
    if (!$valid) {
        $valid = file_exists($classFile = $rootPath . 'app' . DS . 'models' . DS . $className . '.php');
    }
